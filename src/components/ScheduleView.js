import { decimalToBinary, ordinal } from '../lib/encoding'

function ScheduleView({ data, base, labels = [], clock, currentChunk }) {
  let firstLoopStartsAtClock = 14;
  let secondLoopStartsAtClock = 50;

  function focusColor(key, clock) {
    if(clock      + firstLoopStartsAtClock === key && clock < 50 && clock > 1) return 'text-slate-900 font-semibold';
  ...
  return (
    <div className={`duration-500 ${clock < 1 && 'opacity-25'} mt-2 sm:mt-0`}>
      <h2 className="font-semibold my-1 text-slate-100">Message schedule <span className="font-medium text-slate-400">- { ordinal(currentChunk) } chunk</span></h2>
      { data.map((word, key) =>
